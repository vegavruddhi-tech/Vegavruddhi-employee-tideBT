...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      MONTH_NAMES.forEach(month => {
        const ckBt = `ebt_btperf_v9_${month}_${curYear}`;
  ...
  useEffect(() => {
    if (!token) return;
    cachedFetch(`${PROFILE_API_BASE}/api/auth/tidebt-received-payments`, setReceivedPayments, d => d.payments || d || [], 'ebt_payments_v6');
  }, [token, cachedFetch]);
  ...
  useEffect(() => {
    ...
    cachedFetch(
      ...
      d => d,
      `ebt_btperf_v9_${selectedMonth}_${selectedYear}`
    );
    ...
    cachedFetch(
      ...
      d => d,
      `ebt_btperf_v9_${prevMonthName}_${prevYear}`
    );
