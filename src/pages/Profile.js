...
export default function Profile() {
  ...
  // Load task count
  useEffect(() => {
    fetch(`${PROFILE_API_BASE}/api/tasks/my-tasks/count`, { headers: { Authorization: 'Bearer ' + token } })
      .then(r => r.json())
...
const openCamera = async () => {
  ...
  const uploadPhoto = async (file) => {
    ...
    const res = await fetch(`${PROFILE_API_BASE}/api/auth/update-photo`, { method: 'POST', headers: { Authorization: 'Bearer ' + token }, body: fd });
  ...
  const sendPosRequest = async () => {
    ...
    try {
      const res = await fetch(`${PROFILE_API_BASE}/api/auth/request-position`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token }, body: JSON.stringify({ requestedPosition: posNew, reason: posReason }) });
      const data = await res.json();
  ...
  const sendProfRequest = async () => {
    ...
    try {
      ...
      const res = await fetch(`${PROFILE_API_BASE}/api/requests/profile`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token }, body: JSON.stringify({ changes, reason }) });
      const data = await res.json();
