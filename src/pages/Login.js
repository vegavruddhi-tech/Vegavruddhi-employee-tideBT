...
export default function Login() {
  ...
  useEffect(() => {
    // Check if token is passed in URL (from Tide redirect)
  ...
  const onCredential = async (response) => {
    setLoading(true);
    try {
      const res  = await fetch(`${AUTH_API_BASE}/api/auth/google-login`, {
        method: 'POST',
      ...
      const tideBTRes = await fetch(`${AUTH_API_BASE}/api/auth/check-tidebt-access`, {
